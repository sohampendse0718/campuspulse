...
function AnimatedNumber({ value, duration = 1200 }: { value: number; duration?: number }) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (currentStep >= totalSteps) { setDisplayValue(value); clearInterval(timer) }
    }, stepTime)
...
export default function Home() {
  ...
  const [stats, setStats] = useState({ resolutionRate: 0, studentCount: 0 })
  const images = ['/images/1.png', '/images/2.png', '/images/3.png']

  useEffect(() => {
    const t = setInterval(() => setCurrentImg(p => (p + 1) % images.length), 6500)
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (totalCount != null && resolvedCount != null) newStats.resolutionRate = totalCount > 0 ? Math.round((resolvedCount / totalCount) * 100) : 100
        if (profilesCount != null) newStats.studentCount = profilesCount
  ...
  return (
    <div style={{ background: '#090A0F', minHeight: '100vh' }}>
      ...
      {/* Glow blobs */}
      <div style={{ position: 'fixed', top: '20%', left: '10%', width: 500, height: 500, background: 'radial-gradient(circle,#00E5FF08 0%,transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
      <div style={{ position: 'fixed', top: '40%', right: '5%', width: 400, height: 400, background: 'radial-gradient(circle,#8B5CF608 0%,transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          ...
          <Reveal delay={0}>
            <div style={{
              ...
              marginBottom: 32, fontSize: '0.8rem', fontWeight: 500, color: '#00E5FF', letterSpacing: '0.05em',
            }}>
              <Zap size={12} /> GEC CAMPUS ISSUE TRACKER — LIVE
            </div>
          </Reveal>
          ...
          <Reveal delay={80}>
            <h1 style={{
              ...
              <span style={{ background: 'linear-gradient(135deg,#F1F2F7 30%,#9CA3AF)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Report.{' '}</span>
              <span style={{ background: 'linear-gradient(135deg,#00E5FF,#8B5CF6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Resolve.</span>
              <br />
              <span style={{ background: 'linear-gradient(135deg,#F1F2F7 30%,#9CA3AF)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Rebuild.</span>
            </h1>
          </Reveal>

          {/* Sub-text */}
          <Reveal delay={160}>
            <p style={{ fontSize: '1.15rem', color: '#9CA3AF', maxWidth: 560, margin: '0 auto 40px', lineHeight: 1.7 }}>
          ...
          <Reveal delay={240}>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
              <Link href="/report" style={{
                ...
                fontWeight: 700, fontSize: '1rem', textDecoration: 'none', transition: 'all 0.2s ease',
              }}
              ...
              <Link href="/map" style={{
                ...
                fontWeight: 600, fontSize: '1rem', textDecoration: 'none', transition: 'all 0.2s ease',
              }}
                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background='rgba(0,229,255,0.08)'; el.style.boxShadow='0 0 20px rgba(0,229,255,0.15)'; el.style.transform='translateY(-2px)' }}
                onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.background='transparent'; el.style.boxShadow='none'; el.style.transform='none' }}
              >
                View Campus Map
              </Link>
            </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '0 1.5rem 80px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{
            ...
            {/* Main campus image tile */}
            <Reveal direction="left" style={{ gridColumn: 'span 8', gridRow: 'span 3' }}>
              <div style={{
                borderRadius: 16, overflow: 'hidden',
                border: '1px solid #2A2D3D', position: 'relative',
                minHeight: 480,
                ...
                {images.map((src, idx) => (
                  <img
                    key={src} src={src} alt={`GEC Campus ${idx + 1}`}
                    style={{
                ...
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(9,10,15,0.9) 0%,rgba(9,10,15,0.2) 50%,transparent 100%)', zIndex: 1 }} />
                ...
                <div style={{ position: 'absolute', top: 24, right: 24, zIndex: 2, display: 'flex', gap: 6 }}>
                  {images.map((_, idx) => (
                    <div key={idx} style={{
                      width: currentImg === idx ? 24 : 8, height: 8, borderRadius: 4,
            ...
            {/* Stat tile 1 */}
            <Reveal direction="right" delay={0} style={{ gridColumn: 'span 4' }}>
              <div style={{
                ...
                <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(0,229,255,0.15)', border: '1px solid rgba(0,229,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {/* Stat tile 2 */}
            <Reveal direction="right" delay={120} style={{ gridColumn: 'span 4' }}>
              <div style={{
                borderRadius: 16, padding: '24px',
            ...
            {/* Stat tile 3 */}
            <Reveal direction="right" delay={240} style={{ gridColumn: 'span 4' }}>
              <div style={{
                borderRadius: 16, padding: '24px',
                ...
                <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Reveal>
            <div style={{ textAlign: 'center', marginBottom: 56 }}>
              <div style={{
                ...
                color: '#A78BFA', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', marginBottom: 16,
              }}>
                PLATFORM FEATURES
              </div>
              <h2 style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16 }}>
            {[
              ...
              <Reveal key={title} delay={i * 120} direction="up">
                <div style={{
                  padding: '28px 24px', borderRadius: 16, background: bg, border: `1px solid ${border}`,
                  transition: 'all 0.25s ease', height: '100%',
                }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(-4px)'; el.style.boxShadow = `0 8px 30px ${bg}` }}
                  onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'none'; el.style.boxShadow = 'none' }}
                >
                  <div style={{ width: 48, height: 48, borderRadius: 12, marginBottom: 16, background: `${color}20`, border: `1px solid ${color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon size={22} color={color} />
                  </div>
                  <h3 style={{ color: '#F1F2F7', fontWeight: 700, fontSize: '1.05rem', marginBottom: 8, fontFamily: "'Space Grotesk', sans-serif" }}>{title}</h3>
                  <p style={{ color: '#9CA3AF', fontSize: '0.875rem', lineHeight: 1.65, margin: 0 }}>{desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 24 }}>
            {[
              ...
              <Reveal key={n} delay={i * 150} direction="up">
                <div style={{ padding: '32px 28px', borderRadius: 16, background: '#12131C', border: '1px solid #2A2D3D', position: 'relative', height: '100%' }}>
                  <div style={{ fontSize: '3.5rem', fontWeight: 900, lineHeight: 1, color: `${color}20`, fontFamily: "'Space Grotesk', sans-serif", marginBottom: 16 }}>{n}</div>
                  <h3 style={{ color: '#F1F2F7', fontWeight: 700, fontSize: '1.2rem', marginBottom: 10, fontFamily: "'Space Grotesk', sans-serif" }}>{title}</h3>
                  <p style={{ color: '#9CA3AF', fontSize: '0.875rem', lineHeight: 1.65, margin: 0 }}>{desc}</p>
                  <div style={{ position: 'absolute', top: 24, right: 24, width: 8, height: 8, borderRadius: '50%', background: color, boxShadow: `0 0 10px ${color}` }} />
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          <Reveal>
            <div style={{
              ...
              border: '1px solid #2A2D3D', position: 'relative', overflow: 'hidden',
            }}>
              <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
                ...
                <Link href="/feed" style={{
                  ...
                  fontWeight: 600, fontSize: '1rem', textDecoration: 'none', display: 'inline-block',
                  transition: 'all 0.2s ease',
                }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor='rgba(0,229,255,0.3)'; el.style.color='#F1F2F7' }}
                  onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor='#2A2D3D'; el.style.color='#9CA3AF' }}
                >
                  Browse Feed
                </Link>
              </div>
      ...
      {/* ── FOOTER ───────────────────────────────────────────────── */}
      <footer style={{ borderTop: '1px solid #2A2D3D', padding: '32px 1.5rem', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <MapPin size={16} color="#00E5FF" />
            <span style={{ background: 'linear-gradient(135deg,#00E5FF,#8B5CF6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}>
              CampusPulse
            </span>
          </div>
